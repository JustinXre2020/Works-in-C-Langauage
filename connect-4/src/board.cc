...
void DropDiskToBoard(Board& b, DiskType disk, int col) {
  for (unsigned i = 0; i < Board::kBoardHeight; ++i) {
    if (!BoardLocationInBounds(i, col)) {
      throw std::runtime_error("Column is invalid");
    }
  ...
  throw std::runtime_error("Row is invalid");
}
...
bool SearchForWinner(Board& b, DiskType disk, WinningDirection to_check) {
  if (to_check == WinningDirection::kHorizontal) {
    for (unsigned i = 0; i < Board::kBoardHeight; ++i) {
      for (unsigned j = 3; j < Board::kBoardWidth; ++j) {
        if (b.board[i][j] == disk && 
            b.board[i][j] == b.board[i][j - 1] &&
            b.board[i][j] == b.board[i][j - 2] &&
            b.board[i][j] == b.board[i][j - 3]) {
          return true;
        }
      }
    }
  } else if (to_check == WinningDirection::kVertical) {
    for (unsigned i = 3; i < Board::kBoardHeight; ++i) {
      for (unsigned j = 0; j < Board::kBoardWidth; ++j) {
        if (b.board[i][j] == disk && 
            b.board[i][j] == b.board[i - 1][j] &&
            b.board[i][j] == b.board[i - 2][j] && 
            b.board[i][j] == b.board[i - 3][j]) {
          return true;
        }
      }
    }
    ...
    for (unsigned i = 3; i < Board::kBoardHeight; ++i) {
      for (unsigned j = 3; j < Board::kBoardWidth; ++j) {
        if (b.board[i][j] == disk && 
            b.board[i][j] == b.board[i - 1][j - 1] &&
            b.board[i][j] == b.board[i - 2][j - 2] &&
            b.board[i][j] == b.board[i - 3][j - 3]) {
          return true;
        }
      }
    }
    ...
    for (unsigned i = 3; i < Board::kBoardHeight; ++i) {
      for (unsigned j = 3; j < Board::kBoardWidth; ++j) {
        if (b.board[i][j - 3] == disk &&
            b.board[i][j - 3] == b.board[i - 1][j - 2] &&
            b.board[i][j - 3] == b.board[i - 2][j - 1] &&
            b.board[i][j - 3] == b.board[i - 3][j]) {
          return true;
        }
      }
    }
